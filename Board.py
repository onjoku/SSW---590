...
class Game:
    ...
    def __init__(self):
        ...
        self.message = "Prompting a player"
        self.gameboard = {}
        self.placePieces()
        print("Chess game: Enter moves separated by space")
        self.main()
...
class Piece:
    ...
    """
    A piece representing the position of chase pieces
    which comprises of black and white
    """
    ...
    def isValid(self, startpos, endpos, Color, gameboard):
        if endpos in self.availableMoves( startpos[1], gameboard, Color = Color):
            return True
        return False
    ...
    def availableMoves(self, x, y, gameboard):
        print("Invalid: No movement from base class")

    def addnext(self, x, y, gameboard, Color, intervals):
        List_C = []
        for xint, yint in intervals:
            ...
            while self.isInBounds(xtemp, ytemp):
                ...
                if target is None:
                    List_C.append(xtemp, ytemp)
                elif target.Color != Color:
                    List_C.append(xtemp, ytemp)
                    break
                else:
                    break
                xtemp, ytemp = xtemp + xint, ytem + yint
        return List_C
    def isInbounds(self, x, y):
        # Check if there is an empty position on the board
        if x >= 0 and x < 8 and y >= 0 and y < 8:
            return True
        return False

    def positionMismatch(self, gameboard, inittialColor, x, y):
